...
const props = defineProps<{ mode: 'dashboard' | 'catatan' | 'kolam'; [key: string]: any }>();
    ...
    <section v-if="(props.alerts || []).length" class="grid gap-3 md:grid-cols-2">
      <div v-for="a in props.alerts" :key="a.kode" class="rounded-2xl border p-4"
        ...
        <div class="mt-2">
          ...
          <button v-else type="button" class="text-[11px] font-bold text-sky-700" @click="tindakDari(a)">+ Buat tindak lanjut</button>
        </div>
      </div>
    </section>
