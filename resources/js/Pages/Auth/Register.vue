...
<template>
  ...
  <form class="space-y-4" @submit.prevent="daftar">
    ...
    <div><label class="label">Jabatan</label><select v-model="form.position" required class="input"><option value="">— pilih jabatan —</option><option v-for="p in positions" :key="p" :value="p">{{ p }}</option></select></div>
    <div><label class="label">Perusahaan</label><select v-model="form.company_id" class="input"><option value="">— pilih perusahaan —</option><option v-for="c in companies" :key="c.id" :value="String(c.id)">{{ c.name }}</option></select></div>
