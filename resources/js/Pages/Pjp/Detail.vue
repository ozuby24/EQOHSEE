...
async function hapusEvaluasi(e: Record<string, any>) {
  if (!await tanya({
    judul: `Hapus evaluasi ${e.semesterLabel} ${e.tahun}?`,
...
/* Sama seperti di daftar: namanya harus diketik ulang. Yang hilang
   bukan satu baris melainkan seluruh riwayat pemantauan perusahaan itu. */
async function hapusPjp() {
  if (!await tanya({
    judul: `Hapus data PJP “${props.pjp.nama_perusahaan}”?`,
          ...
          <form class="mt-4 grid gap-3" @submit.prevent="simpanEvaluasi">
            <div class="grid grid-cols-2 gap-2">
              ...
              <input v-model.number="evaluasi.tahun" type="number" min="2000" max="2100"
                     aria-label="Tahun evaluasi" placeholder="Tahun"
