...
const props = defineProps<{
  pjp: {
    id: number; nama_perusahaan: string; nib: string | null;
    penanggung_jawab: string | null; alamat: string | null;
...
const form = useForm({
  ...
  status: props.pjp.status,
  catatan: props.pjp.catatan ?? '',
});

function simpan() {
  form.put(`/pjp/${props.pjp.id}`);
}
</script>
...
<template>
  ...
  <div class="max-w-2xl mx-auto">
    <h2 class="font-serif text-xl font-bold text-cam-ink">Ubah Data PJP</h2>
    <p class="text-[12.5px] text-stone-500 mt-1 mb-6">Perbarui data untuk {{ pjp.nama_perusahaan }}.</p>
    ...
    <form class="space-y-5" @submit.prevent="simpan">
      ...
      <div class="flex gap-3">
        ...
        <Link :href="`/pjp/${pjp.id}`" class="rounded-xl border border-stone-200 px-4 py-2.5 text-[12.5px] font-semibold text-stone-600 hover:bg-stone-50">Batal</Link>
      </div>
    </form>
  </div>
</template>
