...
<template>
  ...
  <div class="space-y-5">
    ...
    <section v-if="props.mode === 'rencana'" class="space-y-4">
      ...
      <form @submit.prevent="simpanRencana"><div class="rounded-2xl bg-white border border-stone-100 shadow-card p-5"><h3 class="font-bold text-[14px]">Sembilan komponen rencana audit</h3><div class="grid gap-3 md:grid-cols-2 mt-4"><label v-for="field in ['nomor','tanggal_mulai','tanggal_selesai']" :key="field" class="text-[12px] font-semibold">{{ field.replaceAll('_',' ') }}<input v-model="rencanaForm[field]" :type="field.includes('tanggal') ? 'date' : 'text'" class="mt-1 w-full rounded-xl border-stone-200 text-[12px]"></label><label v-for="field in ['tujuan','kriteria','ruang_lingkup','sumberdaya','metode','sampel']" :key="field" class="text-[12px] font-semibold md:col-span-2">{{ field.replaceAll('_',' ') }}<textarea v-model="rencanaForm[field]" rows="2" class="mt-1 w-full rounded-xl border-stone-200 text-[12px]"></textarea></label></div>

        <!-- Komponen ke-8 punya halamannya sendiri. Dua kotak teks di atas
             tetap ada sebagai ringkasan naratif; yang menjawab "kriteria ini
             dibuktikan dengan apa, sampelnya mana" adalah matriksnya. -->
        <div class="rounded-xl border border-stone-200 p-4 mt-4 flex flex-wrap items-center justify-between gap-3">
          <div class="min-w-0">
            <p class="text-[12.5px] font-bold text-cam-ink">Matriks Metode &amp; Sampel per kriteria</p>
            <p class="text-[11.5px] text-stone-500 mt-1 leading-relaxed">
              <b>{{ props.rekapSampel?.terisi ?? 0 }}</b> dari {{ props.rekapSampel?.total ?? 0 }} kriteria
              sudah punya metode pembuktian<span v-if="props.rekapSampel?.na">, {{ props.rekapSampel.na }}
              dinyatakan tidak berlaku bagi auditi</span>.
              <span v-if="props.rekapSampel?.kurang" class="text-stone-400">
                Sisa {{ props.rekapSampel.kurang }} kriteria belum direncanakan.
              </span>
            </p>
          </div>
          <Link :href="props.tautanSampel" class="eq-btn-utama shrink-0">Buka matriks</Link>
        </div>
